...
public class GpuDriverMetadata {
    ...
    public GpuDriverMetadata() {
        // Empty constructor for system driver
    }
    ...
    void parseJson(@NonNull String jsonString) throws JSONException {
        ...
        if (json.has("driverVersion")) {
            // ADPKG format uses driverVersion - this often contains Vulkan version
            String driverVer = json.getString("driverVersion");
            mVersion = driverVer;
            
            // Extract Vulkan version from driverVersion if it contains "Vulkan"
            if (driverVer.toLowerCase().contains("vulkan")) {
                // Extract version number (e.g., "Vulkan 1.4.330" -> "1.4.330")
                ...
                // If it's just a version number, use it as Vulkan version
                mVulkanVersion = driverVer;
            }
        }
        
        // Try various field names for Vulkan version (if not already set)
        ...
        // If still no Vulkan version, check if version field looks like Vulkan version
        if (TextUtils.isEmpty(mVulkanVersion) && !TextUtils.isEmpty(mVersion)) {
            // Check if version matches Vulkan version pattern (e.g., "1.4.325")
